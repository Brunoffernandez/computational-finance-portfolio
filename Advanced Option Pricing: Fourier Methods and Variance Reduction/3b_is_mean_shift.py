...
for paths in N_Paths:
    ...
    W_q = np.random.normal(mu_shift, np.sqrt(T), paths) # we generate common factor W ~ N(1.5, T) for M paths
    ...
    ratio = np.exp(-(mu_shift / T) * W_q + mu_shiftsq)
    
    #we estimate 95th percentile using weights
    q_95 = weighted_quantile(L_T, ratio, 0.95)
    quantiles_mean.append(q_95)
    
    print(f"{paths:<10} | {q_95:.2f}")


plt.figure(figsize=(8, 5))
plt.plot(N_Paths, quantiles_mean, marker='s', linestyle='-', color='green', label='Estimated 95th pct')
plt.axhline(quantiles_mean[-1], color='gray', linestyle='--', label='Reference (Last value)')
plt.title("Importance Sampling (Mean shift): Convergence of 95% Quantile")
